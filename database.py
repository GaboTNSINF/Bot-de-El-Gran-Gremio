    ...
    async with get_db() as db:
        ...
        if version_db < 1:
            async with transaccion_gremial(db):
                ...
                # IDEMPOTENCIA: Asignación Absoluta.
                await db.execute('''
                ...
                # Creación de Caché Transaccional en memoria O(N)
                await db.execute('''
                ...
                # B-Tree explícito temporal para O(1)
            ...
            # Actualización del metadato del archivo FUERA del bloque rollback
...
async def guardar_registro_matchmaking(user_id: int, rol: str, tier: str, dias: str, inicio: int, fin: int) -> bool:
    try:
        async with get_db() as db:
            async with transaccion_gremial(db):
                await db.execute('''
...
async def registrar_ancla_nomina(seccion: str, channel_id: int, message_id: int) -> bool:
    try:
        async with get_db() as db:
            async with transaccion_gremial(db):
...
async def actualizar_miembro_personal(user_id: int, division: str, rango: str) -> bool:
    try:
        async with get_db() as db:
            async with transaccion_gremial(db):
...
async def remover_miembro_personal(user_id: int) -> bool:
    try:
        async with get_db() as db:
            async with transaccion_gremial(db):
...
async def inyectar_fondos_ignorados(dm_id: int, nombre_dm: str, valor: int, label_voto: str) -> bool:
    try:
        async with get_db() as db:
            async with transaccion_gremial(db):
...
async def registrar_ticket(channel_id: int, user_id: int) -> bool:
    try:
        async with get_db() as db:
            async with transaccion_gremial(db):
...
async def cerrar_ticket_db(channel_id: int) -> bool:
    try:
        async with get_db() as db:
            async with transaccion_gremial(db):
...
async def agregar_producto_db(nombre: str, precio_str: str, costo_pc: int, categoria: str, descripcion: str) -> bool:
    try:
        async with get_db() as db:
            async with transaccion_gremial(db):
                await db.execute('''
...
async def migrar_catalogo_inicial(catalogo_base: dict) -> bool:
    try:
        async with get_db() as db:
            async with transaccion_gremial(db):
...
async def guardar_datos_ficha_nivel20(user_id: int, stats: dict) -> bool:
    try:
        async with get_db() as db:
            async with transaccion_gremial(db):
